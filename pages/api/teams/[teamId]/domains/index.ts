...
export default async function handle(
  ...
  if (req.method === "GET") {
    ...
    try {
      ...
      // Check if domain contains classified or securemi
      if (sanitizedDomain.toLowerCase().includes("classified") || sanitizedDomain.toLowerCase().includes("securemi")) {
        return res
          .status(400)
          .json({ message: "Domain cannot contain 'classified' or 'securemi'" });
      }
