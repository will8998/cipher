...
export default function DomainSection({
  ...
  const isEditingCustomDomain =
          editLink && data.domain && data.domain !== "securemi.xyz" ? true : false;
  ...
  const handleDomainChange = (value: string) => {
    ...
    if (value === "add_domain" || value === "add_dataroom_domain") {
      setModalOpen(true);
              setData({ ...data, domain: "securemi.xyz" });
        setDisplayValue("securemi.xyz");
      return;
    }

            // Check if this is a custom domain selection (not securemi.xyz)
        if (value !== "securemi.xyz") {
  ...
  useEffect(() => {
    if (domains && !editLink) {
      ...
      const domainValue = canUseCustomDomain
        ? (defaultDomain?.slug ?? "securemi.xyz")
        : "securemi.xyz";
  ...
  const defaultDomain = editLink
    ? (data.domain ?? "securemi.xyz")
    : (linkType === "DOCUMENT_LINK" && canUseCustomDomainForDocument) ||
        (linkType === "DATAROOM_LINK" && canUseCustomDomainForDataroom)
      ? (domains?.find((domain) => domain.isDefault)?.slug ?? "securemi.xyz")
      : "securemi.xyz";
  ...
  return (
    <>
        ...
        >
          <SelectTrigger
            className={cn(
              ...
              data.domain && data.domain !== "securemi.xyz"
                ? ""
          ...
          <SelectContent className="flex w-full rounded-md border border-input bg-white text-foreground placeholder-muted-foreground focus:border-muted-foreground focus:outline-none focus:ring-inset focus:ring-muted-foreground dark:border-gray-500 dark:bg-gray-800 focus:dark:bg-transparent sm:text-sm">
            <SelectItem value="securemi.xyz" className="hover:bg-muted">
              securemi.xyz
            </SelectItem>
            {linkType === "DOCUMENT_LINK" && (
              <>
        ...
        {data.domain && data.domain !== "securemi.xyz" ? (
          <Input
            ...
            className={cn(
              ...
              data.domain && data.domain !== "securemi.xyz" ? "flex" : "",
      ...
      {data.domain && data.domain !== "securemi.xyz" && !isDomainVerified ? (
